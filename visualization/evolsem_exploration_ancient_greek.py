...
def extract_key_word(x,word):
    pos_words = x.split(" ")
    for posword in pos_words:
        if re.search(word, posword):
            return posword.split('/')[0]
    return False
def load_tokens(word):
        df = pd.read_csv(wordsdic[word])
        # extract keyword from pos-text
        if "key_word" in df.columns:
            tk = df["key_word"].value_counts().index.tolist()
            tk.insert(0,"Choose a token")
        else:
            df["key_word"]= df.pos_text.apply(lambda x : extract_key_word(x,word))
            df.to_csv(wordsdic[word], index=False)
            tk = df["key_word"].value_counts().index.tolist()
            tk.insert(0,"Choose a token")
        return tk
...
analysis = st.sidebar.radio(
     ...
     'Dependency Analysis (global)[TOBEDONE]'), index=0)
...
# overall analysis
    ...
    if word != 'Choose a lexeme':
        ...
        print("Pickle data (",wordsdic[word],') ', data)
        with st.expander("Contextual embeddings"):
            ...
            st.markdown("**Word : " + word + ", Corpus : Diorisis Ancient Greek, Language model : BERT**")
