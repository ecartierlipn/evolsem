...
#                word_embs2 = pickle.load(open("./input_files/fasttext.fasttext.gallica_corpus.cbow.bin.evolsem_words.pkl", mode="rb"))
            ...
            with st.expander("Contextual embeddings"):
                ...
                with col3:
                    ...
                    sent1 = {sent_score[0]  for tok in data1 for sent_score in data1[tok] if sent_score[1] > 0.5}                
                    ord_dict = [{"lexeme":tok.lower(), "score" : round(sent_score[1],5), "sentence": re.sub('<mask>', '<mark>'+word+'</mark>',sent_score[0])}  for tok in data1 for sent_score in data1[tok] if sent_score[1]>0.5] # if sent_score[0] in sent1 andjust sentences with word as prediction (bast predictions!)
                    uniqw1 = {tok['lexeme']  for tok in ord_dict}                
                    st.info("Gallica data ("+ str(len(uniqw1)) + " words [filtered from "+  str(len(data1))+ "] for " + str(len(sent1)) + " sentences [filtered from " + str(data1count) + "])")
                    ...
                    gb.configure_column('score', aggFunc="max")
                ...
                with col4:
                    sent2 = {sent_score[0]  for tok in data2 for sent_score in data2[tok] if sent_score[1] > 0.5}                
                    ord_dict2 = [{"lexeme":tok.lower(), "score" : round(sent_score[1],5), "sentence": re.sub('<mask>', '<mark>'+word+'</mark>',sent_score[0])}  for tok in data2 for sent_score in data2[tok] if sent_score[1] > 0.5] # just sentences with word as prediction (bast predictions!)
                    uniqw2 = {tok['lexeme']  for tok in ord_dict2}                
                    st.info("JSI data ("+ str(len(uniqw2)) + " words [filtered from "+  str(len(data2))+ "] for " + str(len(sent2)) + " sentences [filtered from " + str(data2count) + "])")
                    df = pd.DataFrame(ord_dict2)
                    # first datagrid
                    gb = GridOptionsBuilder.from_dataframe(df)
                    return_mode_value = DataReturnMode.__members__['FILTERED']
                    update_mode_value = GridUpdateMode.__members__['MODEL_CHANGED']            
            ...
            with st.expander("Contextual embeddings"):
                ...
                # ag grid : keep sentences with good prediction
                sent = {sent_score[0]  for tok in data for sent_score in data[tok] if sent_score[1]>0.5}                
                ord_dict = [{"lexeme":tok.lower(), "score" : round(sent_score[1],5), "sentence": re.sub('<mask>', '<mark>'+word+'</mark>',sent_score[0])}  for tok in data for sent_score in data[tok] if sent_score[1] > 0.5] # just sentences with word as prediction (bast predictions!)
