...
# compute internal patterns (ADJ, NOUN, VERB)
        ...
        if pos == "NOUN":
            ...
            for node in root.descendants(following_only=True):
                if node.upos not in ("PUNCT"):
                    ...
                    strlemma += node.lemma + ' '
                    strpos += node.upos + ' '
            return {'id_sent':sent_id, 'form':strform.strip(),'lemma':strlemma.strip(), 'pos':strpos.strip(),'word_pos':pos}
        elif pos == "ADJ":
            for node in root.descendants(preceding_only=True, add_self=True):
                if node.form==word:
                    strform += node.form + ' '
                    strlemma += node.form + ' '
                    strpos += node.form + ' '
                elif node.upos in ("ADJ","NOUN","ADV"):
                    strform += node.form + ' '
                    strlemma += node.lemma + ' '
                    strpos += node.upos + ' '
            for node in root.descendants(following_only=True):
...
# to be removed replaced by same + _verb, _noun, _adj
...
def retrieve_external_patterns_noun(node,sent):
        ...
        if node.udeprel in ('obj','nsubj','iobj','amod','xcomp','ccomp','csubj'):
            if node.parent.upos in ("VERB","ADJ"):
                return {'id_sent':sent,'lemma':node.parent.lemma, 'form':node.parent.form,"pos":node.parent.upos +' ('+ node.udeprel+ ')'}
...
def retrieve_external_patterns_verb(node, sent):
        ...
        for node2 in node.children(following_only=True):
            if node2.udeprel in ('nsubj','obj','iobj','csubj','xcomp'): 
                if node2.upos in ('PROPN'):
                    ...
                    respos = respos + " " + node2.upos
                    resdep = resdep + " " + node2.udeprel                    
            elif node2.udeprel =='ccomp': # VERB + que PROP
                print(node2.compute_text())
                if len(node2.children(preceding_only=True))>0 and node2.children(preceding_only=True)[0].upos=='SCONJ':
                    resform = resform + " " + node2.children(preceding_only=True)[0].form
                    reslemma = reslemma + " " + node2.children(preceding_only=True)[0].lemma
                    respos = respos + " " + node2.children(preceding_only=True)[0].upos
                    resdep = resdep + " " + node2.children(preceding_only=True)[0].udeprel
                else:           
                    if node2.upos in ('PROPN'):
                        resform = resform + " " + node2.form
                        reslemma = reslemma + " " + node2.upos
                        ...
                        respos = respos + " " + node2.upos
                        resdep = resdep + " " + node2.udeprel                    
            # get case ADP as node3.children (preceding) +  node3 (except for des (de les) économies => économies)
